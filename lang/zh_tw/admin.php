...
/**
 * Strings for component 'admin', language 'zh_tw', branch 'MOODLE_31_STABLE'
 *
 * @package   admin
...
$string['alternativefullnameformat_desc'] = '這會定義姓名將會如何顯示給有檢視全名權限的用戶看(預設是管理員、教師、無編輯權教師)。可以如同"完整名稱格式"的設定一樣使用變數。';
...
$string['availablesearchareas'] = '可用的搜尋區域';
...
$string['commonactivitysettings'] = '共同的設定';
...
$string['configcustomusermenuitems'] = '你可以配置用戶選單的內容(除了登出的連結，它會自動加上去)。每一行以|符號隔開，且包含(1)"語言字串名或成分名"形式的字串或純文字，(2)一個網址 和(3)一個圖示，它不是一個小圖就是網址。你可以用一或多個#符號構成一行來做區隔線。';
...
$string['configextendedusernamechars'] = '啟用這個選項將允許學生在帳號名稱中使用任何字元(這並不會影響它們實際的姓名)。預設值是“否”，限制帳號名稱只能使用小寫字母、數字、底線( _ )、橫線( - )、小數點( . )或( @ )。';
...
$string['configfullnamedisplay'] = '這定義了如何顯示姓名。對於多數單一語言網站，最有效的設定是預設的「名 + 姓」，但也可以選擇隱藏姓或者讓當前的語言包來決定(某些語言有不同的姓名顯示習俗)。';
...
$string['configsupportname'] = '這是一個透過電子郵件或網址來提供一般性協助的人或其他團體的姓名。';
...
$string['enableglobalsearch'] = '啟用整體搜尋';
...
$string['globalsearchmanage'] = '管理整體搜尋';
...
$string['loginpasswordautocomplete_help'] = '關閉此選項，將允許用戶在瀏覽器中儲存他們的帳號密碼。開啟此選項會使用戶無法在瀏覽器上儲存帳號密碼。';
...
$string['navexpandmycourses'] = '在儀錶板頁面上已展開的形式顯示我選修的課程';
$string['navexpandmycourses_desc'] = '若啟用，我的課程將會一開始就以展開的形式顯示在儀錶板上。';
...
$string['pathtogs_help'] = '就Linux系統而言，它應該是 \'/usr/bin/gs\'。就Windows系統來說，它應該像是 \'c:gsbingswin32c.exe\' (請確定在路徑上沒有空格 - 若必要，請複製檔案 \'gswin32c.exe\' 和 \'gsdll32.dll\' 到一個其路徑上沒有空格的新資料夾)';
...
$string['pathtounoconv_help'] = '到 unoconv 文件轉換器的路徑。這是一個可執行的檔案，它可用來轉LibreOffice 軟體所支援的各種文件格式。這是可自由選用的，但若指定了，Moodle將會用它來自動轉換文件格式。這是用來支援以PDF批註作業活動的多種輸入檔案。';
...
$string['taskcontextcleanup'] = '清理的處境';
$string['taskcreatecontexts'] = '建立缺少的處境';
...
$string['taskmessagingcleanup'] = '傳簡訊的後台處理';
$string['taskpasswordresetcleanup'] = '清除重設密碼的請求';
$string['taskplagiarismcron'] = '文章剽竊外掛套件的後台處理';
$string['taskportfoliocron'] = '學習歷程檔案外掛套件後台處理';
$string['taskquestioncron'] = '試題引擎的後台處理';
$string['taskregistrationcron'] = '網站註冊';
$string['tasksendfailedloginnotifications'] = '送出登入失敗的通知';
$string['tasksendnewuserpasswords'] = '送出新的用戶密碼';
$string['tasksessioncleanup'] = '清理舊的對話';
$string['taskstatscron'] = '統計的後台處理';
$string['tasktagcron'] = '標籤的後台處理';
